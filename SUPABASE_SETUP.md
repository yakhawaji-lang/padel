...
بعد إكمال الخطوات 1–5 أعلاه، لرفع الكود وتحديث الموقع:

1. **من مجلد المشروع في الطرفية (Terminal):**
   ```bash
   cd c:\padel\padel
   git add .
   git commit -m "Add Supabase sync: data on all devices"
   git push origin main
   ```
2. إذا كان المشروع مربوطاً بـ Vercel، النشر سيُحدَّث تلقائياً بعد الـ push.
3. **مهم:** أضف متغيرات البيئة في Vercel (الخطوة 5) ثم اضغط **Redeploy** من تبويب Deployments حتى يقرأ Vercel المتغيرات الجديدة.
