...
## إعادة نشر النظام كاملاً (الكود + البيانات)

لإعادة نشر التطبيق بالكامل مع التأكد أن البيانات (النوادي، العروض، الحجوزات، إلخ) تظهر للجميع:

1. **رفع آخر الكود إلى GitHub** (من الطرفية):
   ```bash
   cd c:\padel\padel
   git add .
   git commit -m "إعادة نشر النظام كاملاً"
   git push origin main
   ```
2. **مزامنة البيانات إلى Supabase (اختياري):** إذا أضفت نوادي أو عروضاً محلياً ولم تكن متصلاً بـ Supabase سابقاً، شغّل التطبيق محلياً (`npm run dev`)، ادخل لوحة الإدارة واحفظ مرة واحدة (أو عدّل أي نادٍ واحفظ) حتى تُرفع البيانات إلى Supabase. بعدها أي شخص يفتح الرابط المنشور سيرى نفس البيانات.
3. **إعادة النشر على Vercel:** ادخل **vercel.com** → مشروعك → **Deployments** → النقاط الثلاث بجانب آخر نشر من **main** → **Redeploy**. انتظر حتى يكتمل النشر (Ready).
4. **التحقق:** افتح الرابط العام (مثل `https://padel-ecru-delta.vercel.app`) وتأكد أن الواجهة والبيانات كما تريد.
...
1. **رفع الكود إلى GitHub** (من الطرفية على جهازك):
   ...
   git push origin main
   ```
2. إذا كان المشروع مربوطاً بـ Vercel، النشر سيُحدَّث تلقائياً بعد الـ push. إذا ظل الموقع يعرض نسخة قديمة (مثلاً صفحة العروض تختلف عن المحلية)، ادخل إلى **vercel.com** → مشروعك → **Deployments** → اختر آخر نشر من فرع **main** واضغط **Redeploy** حتى يُبنى من آخر commit.
