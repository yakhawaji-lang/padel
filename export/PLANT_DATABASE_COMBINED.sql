-- ============================================================================
-- PlayTix â€” PLANT_DATABASE_COMBINED.sql (Ù…Ù„Ù Ù…Ø¬Ù…Ù‘Ø¯ Ù„Ø²Ø±Ø§Ø¹Ø© Ù‚Ø§Ø¹Ø¯Ø© Ø¬Ø¯ÙŠØ¯Ø©)
-- ÙŠÙÙˆÙ„Ù‘ÙŽØ¯ ØªÙ„Ù‚Ø§Ø¦ÙŠØ§Ù‹ Ø¨ÙˆØ§Ø³Ø·Ø© scripts/build-transfer-pack.ps1 â€” Ù„Ø§ ØªØ¹Ø¯Ù‘Ù„ ÙŠØ¯ÙˆÙŠØ§Ù‹
-- Ø§Ù„Ø¬Ø²Ø¡ 1: CREATE_ALL_TABLES.sql  |  Ø§Ù„Ø¬Ø²Ø¡ 2: Ø¬Ø¯Ø§ÙˆÙ„/Ø£Ø¹Ù…Ø¯Ø© Ø¥Ø¶Ø§ÙÙŠØ© Ø¢Ù…Ù†Ø© Ù„Ù„ØªÙƒØ±Ø§Ø±
-- Ø¨Ø¹Ø¯ Ø§Ù„Ø§Ø³ØªÙŠØ±Ø§Ø¯: Ø§Ø¶Ø¨Ø· DATABASE_URL ÙˆØ´ØºÙ‘Ù„ npm install Ø¹Ù„Ù‰ Ø§Ù„Ø®Ø§Ø¯Ù…/Ø§Ù„Ø¬Ù‡Ø§Ø² Ø§Ù„Ø¬Ø¯ÙŠØ¯
-- ============================================================================
-- ============================================================================
-- PlayTix � ????? ?? ??????? ??????? ???????? (padel_db / u502561206_padel_db)
-- ============================================================================
-- ?????????: ??? ????? ????? ?? ?????? � CREATE IF NOT EXISTS ?? ???? ??????.
-- ?????? ?? ???????? ???????? (?????? ???) ?? ???? ????? ????: server/db/CLEAR_ALL_DATA.sql
-- ????? ???? ???? ?? ?????? ?????? ?? ??????? ????????? ??? POST /api/data/register-pending-club (??? ????? DATABASE_URL / ????? u502561206_padel_db).
-- ??? ??? ????? ??? ????? ?????? ???? ?????:
--   server/db/migrations/phpmyadmin-u502561206-padel_db-SYNC-legacy-columns.sql
-- ???? GitHub: https://github.com/yakhawaji-lang/padel/blob/main/server/db/CREATE_ALL_TABLES.sql
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS booking_payment_shares (
  ...
  refund_notes VARCHAR(500) NULL,
  refund_acknowledged_at DATETIME NULL,
  removed_at DATETIME NULL,
  member_refund_route VARCHAR(24) NULL,
  member_refund_requested_at DATETIME NULL,
  member_refund_net DECIMAL(10,2) NULL,
  created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
...
('at_club', 1, '{"description":"Pay at club with cash or card"}', 'At club', '????? ?? ??????', 1),
('credit_card', 0, '{"provider":"stripe","publishableKey":"","secretKey":"","webhookSecret":"","description":"Online payment via Visa, Mastercard"}', 'Credit card', '??????? ??????????', 2),
('mada', 0, '{"merchantId":"","apiKey":"","gatewayId":"","description":"???? - ????? ????? ????????"}', 'Mada', '????', 3),
('split', 1, '{"deadlineMinutes":30,"description":"Split payment with other participants"}', 'Split payment', '????? ??????', 4)
...
--   platform_payment_gateways: ??????? Stripe/Mada/??????/???????? (???? ?????? ???????)
--   audit_log: ??? ????? ???



-- ============ End part 1 / begin part 2 migrations ============


-- ============================================================================
-- PlayTix � ????? ????? ????? + ????? ????? (????? MariaDB 10.3.7+ � Hostinger)
-- ????: ???? ??????? ? SQL ? ??? ? ?????
-- ?????? ADD COLUMN IF NOT EXISTS ? CREATE INDEX IF NOT EXISTS ?????? ???? phpMyAdmin ??? ?????/????? ??????
-- ???? GitHub:
