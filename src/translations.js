export const translations = {
  en: {
    title: 'Padel Tournament',
    standings: 'Standings',
    rank: 'Rank',
    team: 'Team',
    wins: 'Wins',
    losses: 'Losses',
    draws: 'Draws',
    gamesWon: 'Games Won',
    gamesLost: 'Games Lost',
    gamesDiff: 'Games Diff',
    matches: 'Matches',
    teams: 'Teams',
    add: '+ Add',
    courts: 'Courts',
    assignMatches: 'Assign Matches',
    court: 'Court',
    startTimer: 'Start Timer (15 min)',
    pause: 'Pause',
    resume: 'Resume',
    reset: 'Reset',
    paused: '(Paused)',
    recordResult: 'Record Result',
    empty: 'Empty',
    timeUp: 'TIME UP!',
    confirmPause: 'Are you sure you want to pause the timer?',
    confirmReset: 'Are you sure you want to reset the timer?',
    timeUpAlert: 'Time is up! Please record the match result.',
    tiedScores: 'Scores are tied! Please enter different scores or play a tiebreaker.',
    needTwoTeams: 'You need at least 2 teams!',
    home: 'Home',
    kingOfCourt: 'King of the Court',
    socialTournament: 'Social Tournament',
    switchMode: 'Switch Mode',
    groupStage: 'Group Stage',
    semiFinal: 'Semi-Final',
    final: 'Final',
    thirdPlace: '3rd & 4th Place',
    qualified: 'Qualified',
    vs: 'vs',
    group: 'Group',
    matchHistory: 'Match History',
    showHistory: 'Show Match History',
    hideHistory: 'Hide Match History',
    score: 'Score',
    winner: 'Winner',
    draw: 'Draw',
    matchSchedule: 'Match Schedule',
    showSchedule: 'Show Match Schedule',
    hideSchedule: 'Hide Match Schedule',
    upcomingMatches: 'Upcoming Matches',
    verifyPairings: 'Verify Team Pairings',
    tournaments: 'Tournaments',
    createTournament: 'Create Tournament',
    manageTournaments: 'Manage Tournaments',
    overview: 'Overview',
    noTournaments: 'No tournaments yet',
    tournamentName: 'Tournament Name',
    deleteTournament: 'Delete Tournament',
    activeTab: 'Active',
    members: 'Members',
    member: 'Member',
    memberName: 'Member Name',
    mobileNumber: 'Mobile Number',
    dateOfBirth: 'Date of Birth',
    email: 'Email',
    addMember: 'Add Member',
    editMember: 'Edit Member',
    totalGames: 'Total Games',
    totalWins: 'Total Wins',
    totalLosses: 'Total Losses',
    totalDraws: 'Total Draws',
    totalPoints: 'Total Points',
    tournamentsPlayed: 'Tournaments Played',
    seasonStats: 'Season Statistics',
    noMembers: 'No members yet',
    editMember: 'Edit Member',
    deleteMember: 'Delete Member',
    selectMembers: 'Select Members',
    teamMembers: 'Team Members',
    tournamentsWon: 'Tournaments Won',
    searchMembers: 'Search Members...',
    selectTeamMembers: 'Select Team Members',
    selectedMembers: 'Selected Members',
    searchPlaceholder: 'Search members...',
    defaultPaymentAmount: 'Default payment amount',
    applyFeeToAllMembers: 'Apply to all',
    memberSelectorPaymentHint:
      'Set fees and use WhatsApp as needed. Under «Other club members», cards appear after 3+ phone digits match, or when the club has marked members as directory favorites (bookmark — saved for the whole club by admins).',
    memberSelectorPhonePlaceholder: 'Type mobile number (3+ digits)…',
    memberSelectorPhoneHint:
      'Other members appear when the number matches, or when they are club directory favorites. Current team members always show above.',
    memberSelectorPickFromContacts: 'Choose from contacts',
    memberSelectorSearchPhone: 'Search',
    memberSelectorContactsNotSupported:
      'Choosing a number from contacts is not available in this browser. Use Chrome on Android (HTTPS), or type the number.',
    memberSelectorContactsFailed: 'Could not read contacts. Check permissions or try again.',
    memberSelectorPinTitle: 'Club directory favorite — show without search (saved for this club by admins)',
    memberSelectorUnpinTitle: 'Remove from club directory favorites',
    memberSelectorClubFavoriteAdminOnly: 'Only the club owner or dashboard admin can change directory favorites. Use Club admin → Members, or log in as club admin here.',
    memberSelectorClubFavoriteViewOnlyTitle: 'Club directory favorite (view only — log in as club admin to change)',
    memberSelectorEmptyCards:
      'No cards to show. Type at least 3 digits of a mobile number, or mark members as club favorites (bookmark) from this list or the admin Members page.',
    sendWhatsApp: 'WhatsApp',
    paymentAmountShort: 'Amount',
    clubReceivedPaymentTitle: 'Club received payment (front desk)',
    memberAckPaymentTitle: 'Member confirmed from their account',
    tournamentPayTrackerShort: 'Payment tracking',
    tournamentPayTrackClubShort: 'Club',
    tournamentPayTrackMemberShort: 'Member',
    tournamentPayTimerPrefix: 'Time left',
    tournamentPayTimerHint:
      'Time left uses the club\'s current tournament payment deadline (Settings > Bookings), from when the booking was locked.',
    tournamentPayTimerExpired: 'Deadline passed',
    tournamentPayBadgeAwaitingPayment: 'Awaiting payment',
    tournamentPayBadgeAwaitingClubConfirmation: 'Paid online — awaiting club confirmation',
    tournamentPayBadgePaidOnlineClubOnly: 'Paid online — club recorded',
    tournamentPayBadgePaidOnlineMemberOnly: 'Paid online — member confirmed',
    tournamentPayBadgeFullyConfirmed: 'Fully confirmed',
    tournamentPayBadgeClubReceivedNeedMember: 'Club received — awaiting member confirmation',
    tournamentPayBadgeMemberConfirmedOther: 'Member confirmed (other payment method)',
    tournamentPayBadgeGuestPaidOnline: 'Guest share paid online',
    tournamentPayMarkPaidAtClub: 'Mark paid',
    tournamentPayMarkPaidAtClubTitle: 'Record payment received at the club (front desk)',
    tournamentRemovePaidMemberConfirm:
      'This participant has a recorded payment. Refund (wallet or cash at desk) before removing from the team?',
    tournamentRemoveRefundWalletQ:
      'Credit the refund to their club wallet? OK = wallet. Cancel = then confirm cash at desk instead.',
    tournamentRemoveRefundCashQ: 'Record cash refund at desk (no wallet credit)?',
    tournamentRemoveRefundCashOnlyGuest:
      'This invite is not linked to a member account — refund can only be recorded as cash at desk. Continue?',
    tournamentRemoveNeedClubAdmin:
      'Log in as this club’s dashboard admin to refund and remove a participant linked to a booking payment.',
    tournamentRemoveUnpaidShareConfirm:
      'Remove from the team and cancel their unpaid payment share on the booking?',
    dragMembersBetweenTeamsHint: 'Drag a member card to another team to move them.',
    tournamentRosterUnassignedBadge: 'Unassigned',
    tournamentRosterPoolTitle: 'Tournament participants',
    tournamentRosterPoolDragHint: 'Drag anyone into a team card below, or onto the unassigned strip to pull them out of a team.',
    tournamentRosterBenchDropHint: 'Drop here to unassign (bench)',
    tournamentRosterTotalLabel: 'Total participant fees',
    tournamentSelectParticipants: 'Add participants & fees',
    tournamentRosterBenchSection: 'On bench (unassigned to a team)',
    tournamentEqualSplitLabel: 'Split total equally',
    tournamentEqualSplitApply: 'Apply split',
    tournamentRosterFavoritesToBench: 'Add selected → bench',
    tournamentRosterTeamDropHint: 'Drag participants from the list above or between team cards. Drop on the bench strip to unassign.',
    tournamentRosterEmptyTeamHint: 'Use «Add participants & fees» above, then drag people here.',
    close: 'Close',
    noMembersFound: 'No members found',
    memberSelectorOnTeam: 'On this team',
    memberSelectorOtherMembers: 'Other club members (search or directory favorites)',
    memberSelectorGuestInviteTitle: 'No club member matches this number',
    memberSelectorGuestInviteHint:
      'Creates a WhatsApp message for tournament participation: bilingual text (King of the Court or Social), your share amount, and clearly labeled tap-to-open links for payment and the club page. After you send it, the guest appears on this team as pending until they register and pay.',
    memberSelectorGuestInviteAutoExplain:
      'We pick the right link automatically: (1) Already on PlayTix but not in this club → join the club and pay. (2) Not on PlayTix yet → register, join the club, and pay.',
    memberSelectorGuestInviteWhatsAppButton: 'Create WhatsApp link & open',
    memberSelectorGuestInviteNeedPhoneDigits: 'Enter a full mobile number (at least 8 digits) before sending an invite.',
    memberSelectorGuestInviteNeedLogin:
      'Log in as a Playtix member (with this club) or as this club’s dashboard admin, or save the tournament booking with an organizer. Then try again.',
    memberSelectorGuestInviteNeedAmount: 'Set a default payment amount greater than zero at the top before sending.',
    memberSelectorGuestInviteAmbiguousPhone:
      'More than one PlayTix account matches this number. Resolve it in admin, then try again.',
    memberSelectorGuestInviteWrongFlowUnregistered:
      'This number already has a PlayTix account. The server should use the join-club link; if you see this, refresh and try again.',
    memberSelectorGuestInviteWrongFlowRegistered:
      'No PlayTix account was found for this number. The server should use the registration link; if you see this, refresh and try again.',
    memberSelectorGuestInviteAlreadyInClub:
      'This number belongs to someone already in the club directory — find them in the list above (search or favorites).',
    memberSelectorClubFavoritesRefresh: 'Refresh list',
    memberSelectorClubFavoritesRefreshTitle: 'Reload club data from the server (updates directory favorites)',
    memberSelectorClubFavoritesEmptyHint:
      'No directory favorites yet. Club admins can mark members with the bookmark in this list or on Members in the dashboard.',
    memberSelectorHelpDetailsTitle: 'Tips: fees, search & payment badges',
    memberSelectorSearchHelpSummary: 'When do members appear in the list?',
    memberSelectorQuickPicksExpand: 'Club directory quick picks',
    memberSelectorQuickPicksCollapse: 'Hide quick picks',
    memberSelectorQuickPicksClosedHint: 'Hidden until opened — then we sync from the server and you can select multiple members.',
    memberSelectorQuickPicksLoading: 'Syncing from server…',
    memberSelectorFavoritesSection: 'Club directory favorites (for quick picks)',
    memberSelectorFavoritesSelectAll: 'Select all',
    memberSelectorFavoritesClearSelection: 'Clear selection',
    memberSelectorFavoritesToThisTeam: 'Add selected → this team',
    memberSelectorFavoritesRoundRobin: 'Distribute selected → round-robin all teams',
    memberSelectorFavoritesSequentialTeams: 'Distribute selected → split evenly across teams',
    memberSelectorFavoritesDistributeHint:
      'Uses the default payment amount per row when set. Members are removed from other teams first (same as drag between teams).',
    memberSelectorFavoritesNoneOnTeam:
      'All directory favorites are already on teams. Remove someone from a team or add more favorites from the admin Members page.',
    tournamentGuestPendingStatus: 'Waiting for registration & payment',
    tournamentGuestPendingRemoveTitle: 'Remove from team list (does not cancel the payment link)',
    tournamentGuestPendingWhatsApp: 'WhatsApp again',
    tournamentGuestPendingWhatsAppTitle: 'Regenerate invite link and open WhatsApp',
    pointsHistory: 'Points History',
    showPointsHistory: 'Show Points History',
    hidePointsHistory: 'Hide Points History',
    match: 'Match',
    date: 'Date',
    pointsEarned: 'Points Earned',
    result: 'Result',
    tournamentJoin: 'Tournament Join',
    selectMember: 'Select Member',
    viewHistory: 'View History',
    noMemberSelected: 'Select a member to view points history',
    standingsTab: 'Standings',
    teamsTab: 'Teams',
    courtsTab: 'Courts',
    historyTab: 'History',
    scheduleTab: 'Schedule',
    stage: 'Stage',
    groupStageLabel: 'Group Stage',
    semiFinalLabel: 'Semi-Final',
    finalLabel: 'Final',
    thirdPlaceLabel: '3rd Place',
    oldTournaments: 'Old Tournaments',
    backToDashboard: 'Back to dashboard',
    selectDate: 'Select Date',
    noTournamentOnDate: 'No tournament found on this date',
    tournamentDetails: 'Tournament Details',
    matchesPlayed: 'Matches Played',
    resetTournament: 'Reset Tournament',
    confirmResetTournament: 'Are you sure you want to reset the tournament? This will clear all teams, matches, and standings, but all records will be preserved in history.',
    resetTournamentConfirm: 'Yes, Reset Tournament',
    tournamentName: 'Tournament Name',
    tournamentDate: 'Tournament Date',
    tournamentType: 'Tournament Type',
    createTournament: 'Create Tournament',
    startNewTournament: 'Start New Tournament',
    tournaments: 'Tournaments',
    currentTournament: 'Current Tournament',
    noTournaments: 'No tournaments yet',
    viewTournament: 'View Tournament',
    tournamentHistory: 'Tournament History',
    selectTournament: 'Select Tournament',
    tournamentDetails: 'Tournament Details',
    matchesPlayed: 'Matches Played',
    status: 'Status',
    active: 'Active',
    completed: 'Completed',
    inProgress: 'In Progress',
    bookings: 'Bookings',
    createRegularBooking: 'Create Regular Booking',
    date: 'Date',
    startTime: 'Start Time',
    endTime: 'End Time',
    resource: 'Resource',
    court: 'Court',
    participants: 'Participants',
    addNewParticipant: 'Add New Participant',
    participant: 'Participant',
    amount: 'Amount',
    paymentMethod: 'Payment Method',
    cash: 'Cash',
    card: 'Card',
    notes: 'Notes',
    save: 'Save',
    cancel: 'Cancel',
    delete: 'Delete',
    edit: 'Edit',
    monday: 'Monday',
    tuesday: 'Tuesday',
    wednesday: 'Wednesday',
    thursday: 'Thursday',
    friday: 'Friday',
    saturday: 'Saturday',
    sunday: 'Sunday',
    accounting: 'Accounting',
    empty: 'Empty',
    totalIncome: 'Total Income',
    totalPaid: 'Total Paid',
    totalPending: 'Total Pending',
    totalUnpaid: 'Total Unpaid',
    paymentHistory: 'Payment History',
    allPayments: 'All Payments',
    filterByDate: 'Filter by Date',
    filterByStatus: 'Filter by Status',
    filterByCourt: 'Filter by Court',
    all: 'All',
    paid: 'Paid',
    partiallyPaid: 'Partially Paid',
    notPaid: 'Not Paid',
    allStatuses: 'All Statuses',
    allCourts: 'All Courts',
    bookingId: 'Booking ID',
    bookingDate: 'Booking Date',
    time: 'Time',
    court: 'Court',
    totalAmount: 'Total Amount',
    paidAmount: 'Paid Amount',
    remainingAmount: 'Remaining Amount',
    paymentStatus: 'Payment Status',
    viewDetails: 'View Details',
    exportReport: 'Export Report',
    noPayments: 'No payments found',
    selectDateRange: 'Select Date Range',
    fromDate: 'From Date',
    toDate: 'To Date',
    summary: 'Summary',
    revenue: 'Revenue',
    pending: 'Pending',
    unpaid: 'Unpaid',
    liveOverview: 'Live Overview',
    totalMembers: 'Total Members',
    upcomingBookings: 'Upcoming Bookings',
    scheduledTournaments: 'Scheduled Tournaments',
    accountingSummary: 'Accounting Summary',
    goToSection: 'Go to section',
    calendarLegend: 'Legend',
    calendarLegendCourtPaid: 'Court · paid',
    calendarLegendCourtPartial: 'Court · partial',
    calendarLegendCourtRefundPending: 'Court · refund requested',
    calendarLegendCourtUnpaid: 'Court · due',
    calendarPaymentRefundPending: 'Refund pending',
    calendarLegendTraining: 'Training',
    calendarLegendKing: 'King tournament',
    calendarLegendSocial: 'Social tournament',
    calendarKindCourt: 'Court booking',
    calendarKindTraining: 'Training session',
    calendarKindTournamentKing: 'King of the Court',
    calendarKindTournamentSocial: 'Social tournament',
    calendarTooltipPlaytomic: 'Playtomic',
    calendarTooltipType: 'Type',
    calendarTooltipDetailsSection: 'Details',
    calendarTooltipPayment: 'Payment',
    calendarTooltipPaidOfTotal: 'Collected',
    calendarTooltipOutstanding: 'Outstanding',
    calendarTooltipCourts: 'Courts',
    calendarClickEdit: 'Click to edit',
    calendarClickDetails: 'Click for details & actions',
    calendarDetailClose: 'Close',
    calendarDetailEdit: 'Edit booking',
    calendarDetailEditSchedule: 'Edit schedule',
    calendarDetailOpenTournament: 'Open tournament',
    calendarDetailPlaytomicReadOnly:
      'This booking is synced from Playtomic. Editing and marking payments here is disabled — use Playtomic to change it.',
    calendarDetailConfirmShare: 'Confirm received at club',
    calendarDetailConfirmAllShares: 'Confirm all shares received',
    calendarDetailConfirmFull: 'Confirm full payment received',
    calendarDetailPaymentShares: 'Payment shares',
    calendarDetailNotes: 'Notes',
    calendarDetailDuration: 'Duration',
    calendarDetailMin: 'min',
    calendarDetailPaid: 'Received',
    calendarDetailRefundPending: 'Refund requested — awaiting club',
    calendarDetailDue: 'Due',
    calendarDetailWhen: 'When',
    calendarDetailTrainingTitle: 'Session',
    calendarDetailCoach: 'Coach',
    calendarDetailTournamentHint: 'Run draws, teams, and scores from the tournament tab after opening.',
    calendarHoverForDetails: 'Hover for a quick preview; click for full details & actions',
    bookingOpenEnded: 'Open-ended (until club closing)',
    bookingOpenEndedHint: 'End time matches closing time for that day; you can still change start time.',
    bookingPhone: 'Phone (for WhatsApp)',
    bookingWhatsAppInvite: 'WhatsApp',
    bookingWhatsAppNeedPhone: 'Add a valid phone number to send a WhatsApp invite.',
    bookingMarkAllPaidAtClub: 'Full payment collected at club (walk-in / desk — no participants required)',
    bookingSharePaymentHint:
      'Members see the booking under My bookings. Guests receive a WhatsApp message with registration and payment instructions.',
    bookingScheduleSection: 'Schedule',
    bookingCourtSection: 'Court & price',
    bookingPeopleSection: 'Participants & payment',
    bookingWorkingHoursHint: 'Prefer times within your usual opening range; all times remain editable.',
    bookingParticipantsOptional:
      'Participants are optional. Record court, time, and amount only — then check “payment collected” if the guest already paid at the desk.',
    clubInvoicesSection: 'Club invoices (ledger)',
    invoicesLoading: 'Loading invoices…',
    invoicesNotInstalled: 'Invoice tables are not installed on the server yet. Run the SQL migration linked in the project docs.',
    noInvoices: 'No invoices in this date range.',
    noInvoicesLedgerTitle: 'No issued invoices in this period',
    noInvoicesLedgerHint:
      'The table below lists court bookings and payment status — not the invoice ledger. Invoices appear here after payment is confirmed when invoicing is enabled.',
    invoiceNumber: 'Invoice #',
    invoiceIssuedAt: 'Issued',
    invoiceCustomer: 'Customer',
    invoiceSource: 'Source',
    invoiceLines: 'Line items',
    invoicePayments: 'Payments recorded',
    noAccountingRecords: 'No accounting records match your filters.',
    atClubPayment: 'At club',
    mada: 'Mada',
    wallet: 'Wallet',
    electronicPayment: 'Electronic',
    confirmCashReceived: 'Confirm payment received',
    copyInvoiceWhatsapp: 'Copy message (WhatsApp)',
    invoiceCreatedShort: 'Invoice issued',
    cancelBookingRow: 'Cancel booking',
    permanentDeleteBooking: 'Delete permanently',
    permanentDeleteInvoice: 'Delete invoice (permanent)',
    confirmPurgeInvoice:
      'Permanently delete this invoice, its line items, and payment records from the database? This cannot be undone.',
  },
  ar: {
    title: 'بطولة هلا بادل',
    standings: 'الترتيب',
    rank: 'المركز',
    team: 'الفريق',
    wins: 'الفوز',
    losses: 'الخسارة',
    draws: 'التعادل',
    gamesWon: 'الأشواط المكتسبة',
    gamesLost: 'الأشواط المفقودة',
    gamesDiff: 'فرق الأشواط',
    matches: 'المباريات',
    teams: 'الفرق',
    add: '+ إضافة',
    courts: 'الملاعب',
    assignMatches: 'تعيين المباريات',
    court: 'الملعب',
    startTimer: 'بدء المؤقت (15 دقيقة)',
    pause: 'إيقاف مؤقت',
    resume: 'استئناف',
    reset: 'إعادة تعيين',
    paused: '(متوقف)',
    recordResult: 'تسجيل النتيجة',
    empty: 'فارغ',
    timeUp: 'انتهى الوقت!',
    confirmPause: 'هل أنت متأكد أنك تريد إيقاف المؤقت مؤقتاً؟',
    confirmReset: 'هل أنت متأكد أنك تريد إعادة تعيين المؤقت؟',
    timeUpAlert: 'انتهى الوقت! يرجى تسجيل نتيجة المباراة.',
    tiedScores: 'النتائج متعادلة! يرجى إدخال نتائج مختلفة أو لعب كسر التعادل.',
    needTwoTeams: 'تحتاج إلى فريقين على الأقل!',
    home: 'الرئيسية',
    kingOfCourt: 'ملك الملعب',
    socialTournament: 'بطولة سوشيال',
    switchMode: 'تبديل الوضع',
    groupStage: 'مرحلة المجموعات',
    semiFinal: 'نصف النهائي',
    final: 'النهائي',
    thirdPlace: 'المركز الثالث والرابع',
    qualified: 'متأهل',
    vs: 'ضد',
    group: 'المجموعة',
    matchHistory: 'سجل المباريات',
    showHistory: 'عرض سجل المباريات',
    hideHistory: 'إخفاء سجل المباريات',
    score: 'النتيجة',
    winner: 'الفائز',
    draw: 'تعادل',
    matchSchedule: 'جدول المباريات',
    showSchedule: 'عرض جدول المباريات',
    hideSchedule: 'إخفاء جدول المباريات',
    upcomingMatches: 'المباريات القادمة',
    verifyPairings: 'التحقق من المباريات',
    tournaments: 'البطولات',
    createTournament: 'إنشاء بطولة',
    manageTournaments: 'إدارة البطولات',
    overview: 'نظرة عامة',
    noTournaments: 'لا توجد بطولات بعد',
    tournamentName: 'اسم البطولة',
    deleteTournament: 'حذف البطولة',
    activeTab: 'نشط',
    members: 'الأعضاء',
    member: 'عضو',
    memberName: 'اسم العضو',
    mobileNumber: 'رقم الجوال',
    dateOfBirth: 'تاريخ الميلاد',
    email: 'البريد الإلكتروني',
    addMember: 'إضافة عضو',
    editMember: 'تعديل العضو',
    totalGames: 'إجمالي المباريات',
    totalWins: 'إجمالي الفوز',
    totalLosses: 'إجمالي الخسارة',
    totalDraws: 'إجمالي التعادل',
    totalPoints: 'إجمالي الأشواط',
    tournamentsPlayed: 'البطولات المشاركة',
    seasonStats: 'إحصائيات الموسم',
    noMembers: 'لا يوجد أعضاء بعد',
    editMember: 'تعديل العضو',
    deleteMember: 'حذف العضو',
    selectMembers: 'اختر الأعضاء',
    teamMembers: 'أعضاء الفريق',
    tournamentsWon: 'البطولات المحققة',
    searchMembers: 'البحث عن الأعضاء...',
    selectTeamMembers: 'اختر أعضاء الفريق',
    selectedMembers: 'الأعضاء المحددون',
    searchPlaceholder: 'البحث عن الأعضاء...',
    defaultPaymentAmount: 'مبلغ الدفع الافتراضي',
    applyFeeToAllMembers: 'تطبيق على الجميع',
    memberSelectorPaymentHint:
      'حدد المبالغ واستخدم واتساب عند الحاجة. تحت «بقية أعضاء النادي» تظهر البطاقات بعد تطابق 3 أرقام من الجوال، أو إذا علّم النادي العضو كمفضّل في الدليل (إشارة مرجعية — تُحفظ للنادي كاملاً من لوحة الأدمن).',
    memberSelectorPhonePlaceholder: 'اكتب رقم الجوال (3 أرقام على الأقل)…',
    memberSelectorPhoneHint:
      'يظهر العضو عند تطابق الرقم، أو إن كان ضمن مفضّلي دليل النادي. أعضاء الفريق الحالي يظهرون دائماً في الأعلى.',
    memberSelectorPickFromContacts: 'اختيار من جهات الاتصال',
    memberSelectorSearchPhone: 'بحث',
    memberSelectorContactsNotSupported:
      'اختيار الرقم من جهات الاتصال غير متاح في هذا المتصفح. استخدم Chrome على أندرويد (HTTPS)، أو اكتب الرقم.',
    memberSelectorContactsFailed: 'تعذّر قراءة جهات الاتصال. تحقّق من الأذونات أو أعِد المحاولة.',
    memberSelectorPinTitle: 'مفضّل دليل النادي — إظهار دون بحث (يحفظه المالك/الأدمن للنادي)',
    memberSelectorUnpinTitle: 'إزالة من مفضّلي دليل النادي',
    memberSelectorClubFavoriteAdminOnly:
      'تغيير مفضّلي الدليل متاح لمالك النادي أو أدمن اللوحة فقط. استخدم «الأعضاء» في لوحة النادي، أو سجّل دخول أدمن هنا.',
    memberSelectorClubFavoriteViewOnlyTitle: 'مفضّل دليل النادي (عرض فقط — سجّل دخول أدمن النادي للتعديل)',
    memberSelectorEmptyCards:
      'لا توجد بطاقات. أدخل 3 أرقام على الأقل من الجوال، أو علّم أعضاءً كمفضّلين (إشارة مرجعية) من هذه القائمة أو صفحة الأعضاء في الإدارة.',
    sendWhatsApp: 'واتساب',
    paymentAmountShort: 'المبلغ',
    clubReceivedPaymentTitle: 'استلام الدفع في النادي (الاستقبال)',
    memberAckPaymentTitle: 'تأكيد الدفع من حساب العضو',
    tournamentPayTrackerShort: 'تتبّع الدفع',
    tournamentPayTrackClubShort: 'النادي',
    tournamentPayTrackMemberShort: 'العضو',
    tournamentPayTimerPrefix: 'متبقٍ',
    tournamentPayTimerHint:
      'الوقت المتبقي يعتمد على مهلة دفع البطولة الحالية في إعدادات النادي (الحجوزات)، منذ تأمين الحجز.',
    tournamentPayTimerExpired: 'انتهت المهلة',
    tournamentPayBadgeAwaitingPayment: 'بانتظار الدفع',
    tournamentPayBadgeAwaitingClubConfirmation: 'مدفوع إلكترونياً — بانتظار تأكيد النادي',
    tournamentPayBadgePaidOnlineClubOnly: 'مدفوع إلكترونياً — سجّل النادي الاستلام',
    tournamentPayBadgePaidOnlineMemberOnly: 'مدفوع إلكترونياً — أكّد العضو',
    tournamentPayBadgeFullyConfirmed: 'مؤكد بالكامل',
    tournamentPayBadgeClubReceivedNeedMember: 'استلم النادي — بانتظار تأكيد العضو',
    tournamentPayBadgeMemberConfirmedOther: 'العضو أكّد (طريقة دفع أخرى)',
    tournamentPayBadgeGuestPaidOnline: 'حصة الضيف مدفوعة إلكترونياً',
    tournamentPayMarkPaidAtClub: 'تسجيل الدفع',
    tournamentPayMarkPaidAtClubTitle: 'تسجيل استلام الدفع في النادي (الاستقبال)',
    tournamentRemovePaidMemberConfirm:
      'هذا المشارك لديه دفع مسجّل. استرداد المبلغ (محفظة أو نقد في الاستقبال) قبل الإزالة من الفريق؟',
    tournamentRemoveRefundWalletQ:
      'إيداع الاسترداد في محفظته في النادي؟ موافق = محفظة. إلغاء = ثم تأكيد الاسترداد نقداً.',
    tournamentRemoveRefundCashQ: 'تسجيل استرداد نقدي في الاستقبال (بدون محفظة)؟',
    tournamentRemoveRefundCashOnlyGuest:
      'هذه الدعوة غير مرتبطة بحساب عضو — يمكن تسجيل الاسترداد نقداً فقط في الاستقبال. المتابعة؟',
    tournamentRemoveNeedClubAdmin:
      'سجّل دخول أدمن لوحة النادي لاسترداد مبلغ وإزالة مشارك مرتبط بدفع على الحجز.',
    tournamentRemoveUnpaidShareConfirm: 'إزالة من الفريق وإلغاء حصة الدفع غير المدفوعة على الحجز؟',
    dragMembersBetweenTeamsHint: 'اسحب بطاقة العضو إلى فريق آخر لنقله.',
    tournamentRosterUnassignedBadge: 'غير مُسند',
    tournamentRosterPoolTitle: 'مشاركو البطولة',
    tournamentRosterPoolDragHint: 'اسحب أي مشارك إلى بطاقة فريق أدناه، أو إلى شريط غير المُسندين لإخراجه من الفريق.',
    tournamentRosterBenchDropHint: 'أفلت هنا لإلغاء الإسناد (مقعد انتظار)',
    tournamentRosterTotalLabel: 'إجمالي رسوم المشاركين',
    tournamentSelectParticipants: 'إضافة المشاركين والرسوم',
    tournamentRosterBenchSection: 'على مقعد الانتظار (غير مُسندين لفريق)',
    tournamentEqualSplitLabel: 'تقسيم إجمالي بالتساوي',
    tournamentEqualSplitApply: 'تطبيق التقسيم',
    tournamentRosterFavoritesToBench: 'إضافة المحددين → مقعد الانتظار',
    tournamentRosterTeamDropHint: 'اسحب المشاركين من القائمة أعلاه أو بين بطاقات الفرق. أفلت على شريط مقعد الانتظار لإلغاء الإسناد.',
    tournamentRosterEmptyTeamHint: 'استخدم «إضافة المشاركين والرسوم» أعلاه ثم اسحب الأشخاص إلى هنا.',
    close: 'إغلاق',
    noMembersFound: 'لم يتم العثور على أعضاء',
    memberSelectorOnTeam: 'في هذا الفريق',
    memberSelectorOtherMembers: 'بقية أعضاء النادي (بحث أو مفضّلو الدليل)',
    memberSelectorGuestInviteTitle: 'لا يوجد عضو بهذا الرقم في دليل النادي',
    memberSelectorGuestInviteHint:
      'يُنشئ رسالة واتساب لمشاركة في بطولة: نص بالعربية والإنجليزية (ملك الملعب أو سوشيال)، مبلغ الحصة، وروابط واضحة «اضغط للفتح» للدفع وصفحة النادي. بعد الإرسال يظهر الضيف في الفريق كمعلّق حتى يتسجّل ويدفع.',
    memberSelectorGuestInviteAutoExplain:
      'يُختار الرابط تلقائياً: (١) مسجّل في المنصة وغير منضمّ للنادي → انضمام للنادي والدفع. (٢) غير مسجّل في المنصة → تسجيل وانضمام للنادي والدفع.',
    memberSelectorGuestInviteWhatsAppButton: 'إنشاء رابط واتساب وفتحه',
    memberSelectorGuestInviteNeedPhoneDigits: 'أدخل رقماً كاملاً للجوال (8 أرقام على الأقل) قبل إرسال الدعوة.',
    memberSelectorGuestInviteNeedLogin:
      'سجّل الدخول كعضو منصة مرتبط بهذا النادي أو كمدير النادي من لوحة التحكم، أو احفظ حجز البطولة مع تحديد منظم. ثم أعِد المحاولة.',
    memberSelectorGuestInviteNeedAmount: 'حدّد مبلغاً افتراضياً أكبر من صفر في الأعلى قبل الإرسال.',
    memberSelectorGuestInviteAmbiguousPhone:
      'أكثر من حساب منصة يطابق هذا الرقم. عالج الأمر من الإدارة ثم أعِد المحاولة.',
    memberSelectorGuestInviteWrongFlowUnregistered:
      'الرقم مسجّل في المنصة. يفترض أن يُستخدم رابط الانضمام؛ إن ظهرت هذه الرسالة حدّث الصفحة وأعد المحاولة.',
    memberSelectorGuestInviteWrongFlowRegistered:
      'لا يوجد حساب بهذا الرقم. يفترض أن يُستخدم رابط التسجيل؛ إن ظهرت هذه الرسالة حدّث الصفحة وأعد المحاولة.',
    memberSelectorGuestInviteAlreadyInClub:
      'هذا الرقم لعضو موجود في دليل النادي — ابحث عنه في القائمة أعلاه أو من المفضّلين.',
    memberSelectorClubFavoritesRefresh: 'تحديث القائمة',
    memberSelectorClubFavoritesRefreshTitle: 'إعادة تحميل بيانات النادي من الخادم (يشمل مفضّلي الدليل)',
    memberSelectorClubFavoritesEmptyHint:
      'لا يوجد مفضّلون في الدليل بعد. يمكن لأدمن النادي تعليم الأعضاء بالإشارة المرجعية من هنا أو من «الأعضاء» في لوحة التحكم.',
    memberSelectorHelpDetailsTitle: 'نصائح: الرسوم، البحث، ورموز الدفع',
    memberSelectorSearchHelpSummary: 'متى يظهر الأعضاء في القائمة؟',
    memberSelectorQuickPicksExpand: 'الاختيار السريع من دليل النادي',
    memberSelectorQuickPicksCollapse: 'إخفاء الاختيار السريع',
    memberSelectorQuickPicksClosedHint: 'مخفي حتى تفتحه — ثم يُزامن من الخادم ويمكنك تحديد عدة أعضاء.',
    memberSelectorQuickPicksLoading: 'جاري المزامنة من الخادم…',
    memberSelectorFavoritesSection: 'مفضّلو دليل النادي (اختيار سريع)',
    memberSelectorFavoritesSelectAll: 'تحديد الكل',
    memberSelectorFavoritesClearSelection: 'إلغاء التحديد',
    memberSelectorFavoritesToThisTeam: 'إضافة المحددين ← هذا الفريق',
    memberSelectorFavoritesRoundRobin: 'توزيع المحددين ← بالتناوب على كل الفرق',
    memberSelectorFavoritesSequentialTeams: 'توزيع المحددين ← تقسيم متساوٍ بين الفرق',
    memberSelectorFavoritesDistributeHint:
      'يُستخدم مبلغ الدفع الافتراضي لكل صف عند ضبطه. يُزال العضو من الفرق الأخرى أولاً (مثل السحب بين الفرق).',
    memberSelectorFavoritesNoneOnTeam:
      'كل مفضّلي الدليل مضافون بالفعل للفرق. أزِل عضواً من فريق أو أضِف مفضّلين من صفحة الأعضاء في الإدارة.',
    tournamentGuestPendingStatus: 'في انتظار تسجيل العضو والدفع',
    tournamentGuestPendingRemoveTitle: 'إزالة من قائمة الفريق (لا يلغي رابط الدفع)',
    tournamentGuestPendingWhatsApp: 'واتساب مرة أخرى',
    tournamentGuestPendingWhatsAppTitle: 'تجديد رابط الدعوة وفتح واتساب',
    pointsHistory: 'سجل النقاط',
    showPointsHistory: 'عرض سجل النقاط',
    hidePointsHistory: 'إخفاء سجل النقاط',
    match: 'المباراة',
    date: 'التاريخ',
    pointsEarned: 'النقاط المكتسبة',
    result: 'النتيجة',
    tournamentJoin: 'انضمام للبطولة',
    selectMember: 'اختر العضو',
    viewHistory: 'عرض السجل',
    noMemberSelected: 'اختر عضواً لعرض سجل النقاط',
    standingsTab: 'الترتيب',
    teamsTab: 'الفِرق',
    courtsTab: 'الملاعب',
    historyTab: 'السجل',
    scheduleTab: 'الجدول',
    stage: 'المرحلة',
    groupStageLabel: 'مرحلة المجموعات',
    semiFinalLabel: 'نصف النهائي',
    finalLabel: 'النهائي',
    thirdPlaceLabel: 'المركز الثالث',
    oldTournaments: 'البطولات القديمة',
    backToDashboard: 'العودة إلى لوحة التحكم',
    selectDate: 'اختر التاريخ',
    noTournamentOnDate: 'لا توجد بطولة في هذا التاريخ',
    tournamentDetails: 'تفاصيل البطولة',
    matchesPlayed: 'المباريات المقامة',
    resetTournament: 'إعادة تعيين البطولة',
    confirmResetTournament: 'هل أنت متأكد من إعادة تعيين البطولة؟ سيتم مسح جميع الفِرق والمباريات والترتيب، ولكن سيتم الحفاظ على جميع السجلات في التاريخ.',
    resetTournamentConfirm: 'نعم، إعادة تعيين البطولة',
    tournamentName: 'اسم البطولة',
    tournamentDate: 'تاريخ البطولة',
    tournamentType: 'نوع البطولة',
    createTournament: 'إنشاء البطولة',
    startNewTournament: 'بدء بطولة جديدة',
    tournaments: 'البطولات',
    currentTournament: 'البطولة الحالية',
    noTournaments: 'لا توجد بطولات بعد',
    viewTournament: 'عرض البطولة',
    tournamentHistory: 'سجل البطولات',
    selectTournament: 'اختر البطولة',
    tournamentDetails: 'تفاصيل البطولة',
    matchesPlayed: 'المباريات المقامة',
    status: 'الحالة',
    active: 'نشط',
    completed: 'مكتمل',
    inProgress: 'قيد التنفيذ',
    bookings: 'الحجوزات',
    createRegularBooking: 'إنشاء حجز منتظم',
    date: 'التاريخ',
    startTime: 'وقت البداية',
    endTime: 'وقت النهاية',
    resource: 'الملعب',
    court: 'الملعب',
    participants: 'المشاركون',
    addNewParticipant: 'إضافة مشارك جديد',
    participant: 'المشارك',
    amount: 'المبلغ',
    paymentMethod: 'طريقة الدفع',
    cash: 'نقدي',
    card: 'بطاقة',
    notes: 'ملاحظات',
    save: 'حفظ',
    cancel: 'إلغاء',
    delete: 'حذف',
    edit: 'تعديل',
    monday: 'الاثنين',
    tuesday: 'الثلاثاء',
    wednesday: 'الأربعاء',
    thursday: 'الخميس',
    friday: 'الجمعة',
    saturday: 'السبت',
    sunday: 'الأحد',
    accounting: 'المحاسبة',
    empty: 'فارغ',
    totalIncome: 'إجمالي الدخل',
    totalPaid: 'إجمالي المدفوع',
    totalPending: 'إجمالي المعلق',
    totalUnpaid: 'إجمالي غير المدفوع',
    paymentHistory: 'سجل المدفوعات',
    allPayments: 'جميع المدفوعات',
    filterByDate: 'تصفية حسب التاريخ',
    filterByStatus: 'تصفية حسب الحالة',
    filterByCourt: 'تصفية حسب الملعب',
    all: 'الكل',
    paid: 'مدفوع',
    partiallyPaid: 'مدفوع جزئياً',
    notPaid: 'غير مدفوع',
    allStatuses: 'جميع الحالات',
    allCourts: 'جميع الملاعب',
    bookingId: 'رقم الحجز',
    bookingDate: 'تاريخ الحجز',
    time: 'الوقت',
    court: 'الملعب',
    totalAmount: 'المبلغ الإجمالي',
    paidAmount: 'المبلغ المدفوع',
    remainingAmount: 'المبلغ المتبقي',
    paymentStatus: 'حالة الدفع',
    viewDetails: 'عرض التفاصيل',
    exportReport: 'تصدير التقرير',
    noPayments: 'لم يتم العثور على مدفوعات',
    selectDateRange: 'اختر نطاق التاريخ',
    fromDate: 'من تاريخ',
    toDate: 'إلى تاريخ',
    summary: 'ملخص',
    revenue: 'الإيرادات',
    pending: 'معلق',
    unpaid: 'غير مدفوع',
    liveOverview: 'نظرة حية',
    totalMembers: 'إجمالي الأعضاء',
    upcomingBookings: 'الحجوزات القادمة',
    scheduledTournaments: 'البطولات المجدولة',
    accountingSummary: 'ملخص المحاسبة',
    goToSection: 'انتقل للقسم',
    calendarLegend: 'دليل الألوان',
    calendarLegendCourtPaid: 'ملعب · مدفوع',
    calendarLegendCourtPartial: 'ملعب · جزئي',
    calendarLegendCourtRefundPending: 'ملعب · طلب استرداد',
    calendarLegendCourtUnpaid: 'ملعب · مستحق',
    calendarPaymentRefundPending: 'استرداد معلّق',
    calendarLegendTraining: 'تدريب',
    calendarLegendKing: 'ملك الملعب',
    calendarLegendSocial: 'بطولة سوشيال',
    calendarKindCourt: 'حجز ملعب',
    calendarKindTraining: 'حصة تدريب',
    calendarKindTournamentKing: 'ملك الملعب',
    calendarKindTournamentSocial: 'بطولة سوشيال',
    calendarTooltipPlaytomic: 'Playtomic',
    calendarTooltipType: 'النوع',
    calendarTooltipDetailsSection: 'التفاصيل',
    calendarTooltipPayment: 'الدفع',
    calendarTooltipPaidOfTotal: 'المحصّل',
    calendarTooltipOutstanding: 'المتبقي',
    calendarTooltipCourts: 'الملاعب',
    calendarClickEdit: 'انقر للتعديل',
    calendarClickDetails: 'انقر للتفاصيل والإجراءات',
    calendarDetailClose: 'إغلاق',
    calendarDetailEdit: 'تعديل الحجز',
    calendarDetailEditSchedule: 'تعديل المواعيد',
    calendarDetailOpenTournament: 'فتح البطولة',
    calendarDetailPlaytomicReadOnly:
      'هذا الحجز متزامن من Playtomic. التعديل وتأكيد الدفع من هنا غير متاح — استخدم Playtomic للتغيير.',
    calendarDetailConfirmShare: 'تأكيد الاستلام في النادي',
    calendarDetailConfirmAllShares: 'تأكيد استلام كل الحصص',
    calendarDetailConfirmFull: 'تأكيد استلام المبلغ كاملاً',
    calendarDetailPaymentShares: 'حصص الدفع',
    calendarDetailNotes: 'ملاحظات',
    calendarDetailDuration: 'المدة',
    calendarDetailMin: 'دقيقة',
    calendarDetailPaid: 'مستلم',
    calendarDetailRefundPending: 'تم طلب الاسترداد — بانتظار تنفيذ النادي',
    calendarDetailDue: 'مستحق',
    calendarDetailWhen: 'الموعد',
    calendarDetailTrainingTitle: 'الحصة',
    calendarDetailCoach: 'المدرب',
    calendarDetailTournamentHint: 'إدارة الفرق والمباريات والملاعب من تبويب البطولة بعد الفتح.',
    calendarHoverForDetails: 'مرّر للمعاينة السريعة؛ انقر للتفاصيل والإجراءات',
    bookingOpenEnded: 'حجز مفتوح حتى نهاية الدوام',
    bookingOpenEndedHint: 'يطابق وقت الإغلاق اليومي، ويمكنك تعديل وقت البداية بحرية.',
    bookingPhone: 'الجوال (للواتساب)',
    bookingWhatsAppInvite: 'واتساب',
    bookingWhatsAppNeedPhone: 'أضف رقم جوال صالح لإرسال دعوة واتساب.',
    bookingMarkAllPaidAtClub: 'تم استلام المبلغ كاملاً في النادي (حضور / استقبال — دون إجبار إضافة مشاركين)',
    bookingSharePaymentHint:
      'يظهر الحجز للأعضاء المسجلين في «حجوزاتي». يُرسل للضيف رابط تسجيل واستكمال الدفع عبر واتساب.',
    bookingScheduleSection: 'الجدولة',
    bookingCourtSection: 'الملعب والسعر',
    bookingPeopleSection: 'المشاركون والدفع',
    bookingWorkingHoursHint: 'يُفضّل ضمن أوقات العمل المعتادة؛ ويبقى تعديل جميع الأوقات ممكناً.',
    bookingParticipantsOptional:
      'المشاركون اختياريون. يمكنك حفظ الملعب والوقت والمبلغ فقط — ثم تفعيل «تم استلام المبلغ» إذا دفع في الاستقبال.',
    clubInvoicesSection: 'فواتير النادي (سجل محاسبي)',
    invoicesLoading: 'جاري تحميل الفواتير…',
    invoicesNotInstalled: 'جداول الفوترة غير مثبتة على السيرفر بعد. نفّذ ملف التهجير SQL الموثّق في المشروع.',
    noInvoices: 'لا توجد فواتير ضمن نطاق التاريخ.',
    noInvoicesLedgerTitle: 'لا توجد فواتير مُصدَّرة في هذا النطاق',
    noInvoicesLedgerHint:
      'الجدول أدناه يعرض حجوزات الملاعب وحالة الدفع — وليس سجل الفواتير. تظهر الفواتير هنا بعد تأكيد الدفع عند تفعيل الفوترة.',
    invoiceNumber: 'رقم الفاتورة',
    invoiceIssuedAt: 'تاريخ الإصدار',
    invoiceCustomer: 'العميل',
    invoiceSource: 'المصدر',
    invoiceLines: 'البنود',
    invoicePayments: 'حركات القبض',
    noAccountingRecords: 'لا توجد سجلات محاسبية تطابق التصفية.',
    atClubPayment: 'في النادي',
    mada: 'مدى',
    wallet: 'المحفظة',
    electronicPayment: 'إلكتروني',
    confirmCashReceived: 'تأكيد استلام الدفع',
    copyInvoiceWhatsapp: 'نسخ رسالة (واتساب)',
    invoiceCreatedShort: 'أُصدرت الفاتورة',
    cancelBookingRow: 'إلغاء الحجز',
    permanentDeleteBooking: 'حذف نهائي',
    permanentDeleteInvoice: 'حذف الفاتورة نهائياً',
    confirmPurgeInvoice:
      'حذف هذه الفاتورة وبنودها وحركات القبض نهائياً من قاعدة البيانات؟ لا يمكن التراجع.',
  }
}

