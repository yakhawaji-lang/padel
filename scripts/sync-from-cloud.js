#!/usr/bin/env node
/**
 * مزامنة البيانات من السحابة إلى PostgreSQL المحلي
 * يستخدم DATABASE_URL (محلي) و DATABASE_URL_CLOUD (سحابة)
 *
 * الاستخدام:
 *   DATABASE_URL=postgresql://...localhost... DATABASE_URL_CLOUD=postgresql://...neon... node scripts/sync-from-cloud.js
 */
import pg from 'pg'
...
if (!localUrl || !cloudUrl) {
  ...
  console.error('مثال: DATABASE_URL_CLOUD=postgresql://user:pass@host/db node scripts/sync-from-cloud.js')
  process.exit(1)
}
...
async function sync() {
  ...
  // app_store: upsert by key (cloud → local)
  const { rows: storeRows } = await cloud.query('SELECT key, value, updated_at FROM app_store')
  ...
  // matches, member_stats, tournament_summaries: truncate and copy
  const tables = [
  ...
  for (const [table, cols] of tables) {
    const { rows } = await cloud.query(`SELECT ${cols} FROM ${table}`)
    await local.query(`TRUNCATE ${table} RESTART IDENTITY CASCADE`)
    for (const r of rows) {
  ...
  await local.end()
  await cloud.end()
  console.log('\nتمت المزامنة بنجاح من السحابة إلى المحلي.')
}
