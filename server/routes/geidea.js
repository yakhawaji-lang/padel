 ...
 *  POST /api/payments/geidea/callback          -> Geidea server-to-server callback (webhook)
 *
...
router.get('/config', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: e?.message || 'config_error' })
  }
})
...
router.post('/session', sessionLimiter, async (req, res) => {
  const { bookingId, returnUrl, callbackUrl, customer, amount: amountOverride } = req.body || {}
  ...
  if (bookingId) {
    try {
      ...
      console.warn('geidea /session lookup:', e?.message)
  ...
  try {
    ...
    res.json({ ok: true, ...result, publicKey: publicCfg.publicKey })
  } catch (e) {
    const code = e?.code || 'GEIDEA_ERROR'
...
/**
 * Geidea server-to-server callback.
 * Geidea POSTs payment results here. We log it and (when possible) advance
 * the associated booking. Production hardening should also verify the
 * X-Signature header.
 */
router.post('/callback', async (req, res) => {
  try {
    const payload = req.body || {}
    const status = String(payload?.order?.status || payload?.status || '').toLowerCase()
    const merchantRef = payload?.order?.merchantReferenceId || payload?.merchantReferenceId
    const bookingId = (() => {
      if (typeof merchantRef === 'string' && merchantRef.startsWith('BK-')) {
        return Number(merchantRef.split('-')[1])
      }
      return null
    })()
    ...
    if (bookingId && (status === 'success' || status === 'paid' || status === 'captured')) {
      try {
        ...
        console.warn('geidea /callback update booking:', e?.message)
      }
    }
    res.json({ ok: true })
  } catch (e) {
