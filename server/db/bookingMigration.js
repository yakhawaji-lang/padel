...
export async function runMigration() {
  ...
  const bpsRefundCols = [
    ...
    ['refund_acknowledged_at', 'DATETIME'],
    ['removed_at', 'DATETIME']
  ]
  for (const [col, type] of bpsRefundCols) {
    if (!(await columnExists('booking_payment_shares', col))) {
      await query(`ALTER TABLE booking_payment_shares ADD COLUMN \`${col}\` ${type} NULL`)
    }
  }
  ...
  if (!(await tableExists('club_directory_favorites'))) {
    await query(`
      CREATE TABLE club_directory_favorites (
        club_id VARCHAR(255) NOT NULL,
        member_id VARCHAR(255) NOT NULL,
        created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
  ...
  if (!(await tableExists('coach_training_invites'))) {
    await query(`
      CREATE TABLE coach_training_invites (
        id INT AUTO_INCREMENT PRIMARY KEY,
        club_id VARCHAR(255) NOT NULL,
        booking_id VARCHAR(255) NOT NULL,
