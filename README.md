...
## نشر على Hostinger (GitHub + MySQL)

للرفع إلى GitHub وربط المشروع مع Hostinger وزراعة قاعدة البيانات:

1. راجع [docs/DEPLOY_GITHUB_HOSTINGER_COMPLETE.md](./docs/DEPLOY_GITHUB_HOSTINGER_COMPLETE.md)
2. أنشئ `database.config.json` في `domains/playtix.app/` (خارج public_html) — انسخ من `database.config.json.example`
3. اتبع الخطوات: GitHub → Hostinger → MySQL → تهيئة الجداول

**مهم:** `database.config.json` خارج `public_html` حتى لا يتأثر بالتحديثات من Git.
